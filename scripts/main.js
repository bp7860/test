	...
	if (item) {
		...
		$.each(item.equipment, function (key, val) {
			...
			if( key%3 == 0 )
				...
				grid = 'c';
			equipment_tmp+='<div class="ui-block-'+grid+'"><a href="'+window.location.hash +'&popupBasic_'+ val +'" data-rel="popup" class="ui-btn ui-corner-all ui-shadow ui-btn-inline" data-transition="pop">Basic Popup</a><div data-role="popup" id="popupBasic_'+ val +'"><p>' + legende[Lang][(val-1)] + '</p></div></div>';
			equipment_msg+='';
		});
